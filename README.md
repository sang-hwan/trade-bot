...
### `monitoring/trade_log_viewer.py` 매매 로그 시각화
- 하는 일: 실시간으로 수집되는 매매 로그(`trades.jsonl`)를 읽어, 종목·시간·가격·수량·수수료·사유(신호/스탑/리밸런스 등)를 테이블 형태로 표시  
- 왜 필요한가: 콘솔 로그 없이도 현재 어떤 거래가 체결되고 있는지 직관적으로 확인하기 위함  
- 언제 쓰나요: 실매매 중 상시 실행 (3~5초 간격으로 자동 새로고침)  

### `monitoring/portfolio_viewer.py` 포트폴리오 실시간 시각화
- 하는 일: 보유 종목의 수량·평단·현재가·평가손익을 집계해 **포트폴리오 비중 차트**와 **종목별 손익표**를 시각적으로 표시  
- 왜 필요한가: 현재 자산이 어디에 얼마나 투자되어 있고 어떤 종목이 손익을 주도하는지 한눈에 파악하기 위함  
- 언제 쓰나요: 장중 실시간 확인, 리밸런싱 또는 스탑 트리거 감시 중  

### `monitoring/equity_curve_viewer.py` 수익 실시간 시각화
- 하는 일: 실시간 `equity_curve.csv`를 기반으로 누적 수익률, 일중 P/L, MDD를 라인 차트로 표시하고, 목표 대비 편차를 시각적 배지로 표시  
- 왜 필요한가: 수익 곡선과 드로우다운 추이를 실시간으로 관찰하여 과도한 변동성이나 손실 확대를 즉시 인지하기 위함  
- 언제 쓰나요: 장중 상시 실행 (3~5초 간격 자동 갱신)  

### `monitoring/system_tracker.py` 시스템 문제 추적
- 하는 일: 브로커 API 연결·인증 상태, 레이트 리밋, 시계 동기화, 거래 캘린더·시가 도달 여부, 이상 로그 등을 카드·배지 형태로 표시  
- 왜 필요한가: 거래 시스템의 안정성과 신뢰성을 유지하기 위해 실시간으로 **“지금 시스템이 정상인가?”**를 시각적으로 확인하기 위함  
- 언제 쓰나요: 장 전·장중·장 마감 직전까지 상시 실행하여 즉시 대응 가능하게 함  
